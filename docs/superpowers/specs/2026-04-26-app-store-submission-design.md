...
| Subtitle | 30자 | `AI workspace, local-first` (24자) | `AI 워크스페이스, 로컬 퍼스트` (16자) |
...
- 앱 이름·subtitle·카테고리는 이미 인덱싱 → 중복 제외 (`Hearth`, `productivity`, `workspace`, `local`, `ai`)
- Subtitle "AI workspace, local-first"가 `workspace`, `local`, `first`, `ai`를 인덱싱하므로 키워드에서 제외하여 글자 효율 확보
- 단어 토큰화 — `brain` 단독으로 "second brain" 검색에 매칭됨

**en-US (`docs/superpowers/app-store/keywords-en-US.txt`):**
```
todo,notes,memo,tasks,calendar,planner,brain,palette,capture,sticky,unified,daily,gtd,crm
```
글자 수: 95/100. 만약 subtitle 변경 시 재조정 필요.

**ko-KR (`docs/superpowers/app-store/keywords-ko-KR.txt`):**
```
할일,메모,일정,캘린더,투두,노트,세컨드브레인,명령팔레트,빠른메모,퀵캡처,시간관리,일정관리,데일리플래너,스티키메모,마인드맵,프로젝트관리
```
글자 수: 99/100.

**ko-KR subtitle 중복 회피**: subtitle "AI 워크스페이스, 로컬 퍼스트"가 `AI`, `워크스페이스`, `로컬`, `퍼스트`를 인덱싱하므로 키워드에서 `로컬퍼스트`, `통합워크스페이스`, `AI비서`는 의도적 제외. 대신 `데일리플래너`, `스티키메모`, `마인드맵`, `프로젝트관리`로 대체.
...
| 4 | "내 데이터는 내 Mac에" / "Your data stays on your Mac" | 설정 화면 + 로컬 SQLite 경로 visible | 로컬 퍼스트 (프라이버시) |
...
```
[0:00-0:05]  Hearth 로고 페이드인 + 카피 "AI workspace, local-first"
[0:05-0:11]  메인 화면 (탭 3개 데이터 풍성)
             카피: "Projects, memos, schedules — one place"
[0:11-0:18]  ⌘K 팔레트 → 사용자 타이핑 → AI 응답 → 실제 일정 추가
             카피: "⌘K — AI command palette"
[0:18-0:23]  ⌃⇧H Quick Capture 오버레이 → Enter → 메모 저장
             카피: "⌃⇧H — capture from anywhere"
[0:23-0:28]  ⌘F 검색 → 결과 카드 → 클릭 → 앰버 글로우 펄스
             카피: "⌘F — find anything"
...
5. OPTIONAL FEATURES (not required for review)
   ...
   Both integrations are clearly opt-in via Settings → Integrations.
